...
(function () {
  ...
  function reveal(note){
    requestAnimationFrame(function(){
      document.documentElement.style.visibility = '';
      document.documentElement.style.opacity = '';
      try { window.ReactNativeWebView.postMessage(JSON.stringify({ tag:'ARTICLES_DEBUG', msg:'READY', extra:{via:note} })); } catch(e){}
    });
  }
  ...
  function safeHideHeaders(keep){
    ...
    KILL_SELECTORS.forEach(function(sel){
      document.querySelectorAll(sel).forEach(function(n){
        // Skip if this node is the keep node, its ancestor, or contains the keep node
  ...
  // Retry quickly to find keepNode; reveal even if not found (fallback)
  var attempts = 0, maxAttempts = 60; // ~3.6s
  (function tick(){
    var keep = findKeepNode();
    if (keep){
      // Do a targeted hide that won't remove content
      safeHideHeaders(keep);
      reveal('kept:' + keep.sel);
      // Keep cleaning late injections that are NOT ancestors of content
    ...
    if (++attempts >= maxAttempts){
      // We didn't find a keep container—show the page anyway (no blank screen)
      reveal('fallback');
      return;
    }
    setTimeout(tick, 60);
          ...
          <>
            {loading && (
              <View
                ...
                style={{
                  position: "absolute",
                  top: headerHeight ?? 56,   // or 0 if you prefer
                  left: 0,
                  right: 0,
                  paddingVertical: 8,
                  alignItems: "center",
            ...
            <WebView
              ...
              // @ts-ignore
              cacheMode="LOAD_NO_CACHE"
              onMessage={(e) => console.log("[ArticlesWebView]", e.nativeEvent.data)}
            />
          </>
        )}
      </SafeAreaView>
