...
export function useActivityReplies() {
  ...
  const seq = useRef<Record<number, number>>({}); // per-thread sequence guard
    ...
    async (id: number) => {
      ...
      const p = (async () => {
        ...
        try {
          ...
          // ignore stale response for this thread
          if (seq.current[id] !== mySeq) return;
  ...
  const submit = useCallback(
    async (id: number, override?: string) => {
      ...
      setSubmitErrors(prev => ({ ...prev, [id]: undefined })); // clear before submit
